...
- 웹서버는 모듈화된 정적 파일을 제공하며 두 미러로 이동할 수 있는 링크를 제공합니다.
...
1. 웹서버의 정적 웹 루트에 저장소의 모든 정적 파일과 디렉터리를 배치합니다.
2. 웹서버 도메인이 `index.html`을 기본 페이지로 제공하도록 설정합니다.
...
6. 웹서버에서 두 미러 링크와 컴포넌트 로딩이 정상적으로 동작하는지 확인합니다.

## 로컬 확인

브라우저의 `file://` 접근은 컴포넌트 `fetch()`가 제한될 수 있으므로 로컬 정적 서버로 확인합니다.
