...
Keimirror는 `mirror.keiminem.com`과 `mirror2.keiminem.com`을 한곳에서 안내하는 정적 HTML 허브 페이지입니다.
...
- `components/cards/`: 환영 문구, 면책 조항, 서버 프로필, 접속자 IP, 미러 접속 정보 등 카드 단위 HTML을 보관합니다.
...
3. `assets/` 및 `components/` 경로의 정적 파일에 같은 도메인에서 접근할 수 있는지 확인합니다.
4. 미러 1 서버는 `mirror.keiminem.com` 도메인에서 fancyindex 기반 기본 디렉터리 목록을 제공합니다.
5. 미러 2 서버는 `mirror2.keiminem.com` 도메인에서 fancyindex 기반 기본 디렉터리 목록을 제공합니다.
